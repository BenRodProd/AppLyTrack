...
const MainList = ({ user }) => {
  ...
  useEffect(() => {
    ...
    if (sorting === 'date') {
      const sortedByDate = [...jobs].sort((a, b) => a.timestamp - b.timestamp);
    ...
    if (sorting === 'state') {
      const sortedByState = [...jobs].sort((a, b) => {
        ...
        return (
          states[a.abgelehnt ? 'dismissed' : a.beworben ? 'applied' : 'none'] -
    ...
    <>
      <Header>
        ...
        <TopStyle>
          ...
          <div>
            <StyledButton onClick={() => setStatsActive((prevState) => !prevState)} type="button">
              <img src={stats} alt="stats" width="70px" />{' '}
            </StyledButton>
          </div>
          <div>
      ...
      {addNewJob && <AddNewJob setAddNewJob={setAddNewJob} addJob={addJob} />}
      <MainStyle>
